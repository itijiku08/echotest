...
def run(
    ...
    if weights is not None:
        ...
        # DataParallelを使用している場合の対応
        if "module." in list(checkpoint['state_dict'].keys())[0]:
            # DataParallelで保存された場合はそのままロード
    ...
    mean, std = echonet.utils.get_mean_and_std(echonet.datasets.Echo(external_test_location=data_dir, split="EXTERNAL_TEST"))
    tasks = []
    ...
    with open(os.path.join(output, "log.csv"), "a") as f:
        if run_test:
            ...
            for split in ["EXTERNAL_TEST"]:
                dataset = echonet.datasets.Echo(external_test_location=data_dir, split=split, **kwargs)
    ...
    if save_video and not all(os.path.isfile(os.path.join(output, "videos", f)) for f in dataloader.dataset.fnames):
        ...
        print(f"Created directory for saving videos: {os.path.join(output, 'videos')}")  # <-- デバッグ出力
        ...
        with torch.no_grad():
            with open(os.path.join(output, "size.csv"), "w") as g:
                ...
                for (x, filenames, length) in tqdm.tqdm(dataloader):
                    ...
                    print(f"After list conversion: {filenames}")
                    ...
                    filenames = filenames#[list(f) for f in filenames][0]

                    start = 0
                    x = x.numpy()
                    for (i, (filename, offset)) in enumerate(zip(filenames, length)):
                        original_filename = "".join(filename)
                        ...
                        # この部分を修正
                        save_path = os.path.join(output, "videos", f"{filename}.avi")
                        echonet.utils.savevideo(save_path, video, 50)
...
def main():
    # ビデオファイルが格納されているディレクトリへのパス
    data_dir = 'videos'

    # `videos` ディレクトリ内のすべての '.avi' ファイルのリストを取得
    video_files = [f for f in os.listdir(data_dir) if f.endswith('.avi')]

    # 各ビデオファイルに対して処理を実行
    for video_file in video_files:
        video_path = os.path.join(data_dir, video_file)
        run(data_dir='videos', output='videos_seg')
